...
const SKILLS: { key: Skill; label: string; help: string }[] = [
  { key: "levantamento", label: "Levantamento", help: "Levantar a bola para o ataque do parceiro." },
  { key: "passe", label: "Passe", help: "Receber o saque do adversário e deixar a bola pronta para o levantamento." },
  { key: "ataque", label: "Ataque", help: "Atacar o adversário, seja por cortada, pingo ou batida." },
  { key: "saque", label: "Saque", help: "Sacar a bola com precisão e consistência." },
];
...
export default function Home() {
  ...
  const missingSkills = current && !skipped[current] ? SKILLS.filter((skill) => typeof draft[current]?.[skill.key] !== "number") : [];
  ...
  function isSkillMissing(skill: Skill) {
    return Boolean(showMissing && current && !skipped[current] && typeof draft[current]?.[skill] !== "number");
  }
  ...
  function warnAboutMissingSkills() {
    ...
    window.requestAnimationFrame(() => {
      setShake(true);
      document.querySelector(".missing-skill")?.scrollIntoView({ behavior: "smooth", block: "center" });
    });
    ...
    {view === "votar" ? <section className={voter ? "content assessment-content" : "content two-col"}>
      ...
      <div className={shake ? "panel validation-shake" : "panel"}>
        {!voter ? <><div className="panel-head"><div><span className="step">PASSO 1</span><h2>Digite seu código</h2></div></div><p>Use os 6 números recebidos pelo WhatsApp. O código fica salvo neste celular.</p><label className="select-label pin-label">Código individual<input value={pin} inputMode="numeric" autoComplete="one-time-code" maxLength={6} placeholder="000000" onChange={(event) => setPin(event.target.value.replace(/\D/g, ""))} /></label><button className="primary full" disabled={saving} onClick={() => void identifyVoter()}>{saving ? "Validando…" : "Continuar"}</button></> : <>
          ...
          <div className="skill-list">{SKILLS.map((skill) => <div className={isSkillMissing(skill.key) ? "skill-row missing-skill" : "skill-row"} key={skill.key}><div><b>{skill.label}</b><small>{skill.help}</small>{isSkillMissing(skill.key) && <em className="missing-label">Preencha este fundamento</em>}</div><div className="score-buttons">{[1, 2, 3, 4, 5].map((score) => <button type="button" key={score} className={current && draft[current]?.[skill.key] === score ? "selected" : ""} onClick={() => setScore(skill.key, score)}>{score}</button>)}<button type="button" className="clear" onClick={() => setScore(skill.key)}>×</button></div></div>)}</div>
